...
for x = 1: length(activities)
    ...
    for k = 1 : length(theFiles)
        ...
        for i=1:30:size(magAccel,1)-window
           ...
           [apks,alocs] = findpeaks(currentA, time,'MinPeakProminence',1);
           [gpks,glocs] = findpeaks(currentG, time,'MinPeakProminence',1);
           ...
           stdPAmpA = std(currentA(locs)); %standard deviation of the amplitude of the peaks for Accel magnitude
           meanPAmpA = mean(currentA(locs)); %mean of the amplitude of the peaks for Accel magnitude
           stdPAmpG = std(currentG(locs)); % standard deviation of the amplitude of the peaks for Gyro magnitude
           meanPAmpG = mean(currentG(locs));
           ...
           fv = [fv; meanPDistA stdPDistA meanPDistG stdPDistG stdPAmpA meanPAmpA stdPAmpG meanPAmpG rmsAmpA rmsAmpG];
        end
%print to file
        activityV = [activityV; fv];
        
    end
    afile = strcat(activities(x),'.csv');
    outfile = strcat(outFolder,afile);
    csvwrite(outfile,activityV);
end
